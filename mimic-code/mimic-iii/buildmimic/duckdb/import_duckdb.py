...
def main() -> int:
        ...
        try:
            ...
            for f in os.listdir(mimic_data_dir):
                ...
                if m is not None:
                    ...
                    print(f"  {tablename}")
                    connection.execute(f"COPY {tablename} from '{os.path.join(mimic_data_dir,m.group(0))}' (FORMAT CSV, DELIMITER ',', HEADER, ignore_errors true);")
    ...
    if make_concepts:
        ...
        try:
            ...
            connection.execute(f"COPY ccs_multi_dx from '{csvgz_path}' (FORMAT CSV, DELIMITER ',', HEADER, ignore_errors true);")
