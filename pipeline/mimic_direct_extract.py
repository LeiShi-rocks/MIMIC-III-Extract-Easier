...
if __name__ == '__main__':
    ...
    if (args['extract_numerics'] == 0 | (args['extract_numerics'] == 1) ) & isfile(os.path.join(outPath, dynamic_hd5_filename)):
        ...
        icuids_to_keep = get_values_by_name_from_df_column_or_index(data, 'icustay_id')
        icuids_to_keep = sorted(set([str(s) for s in icuids_to_keep]), key=int)
        ...
        chartitems_to_keep = sorted(set([ str(i) for i in chartitems_to_keep ]), key=int)

        labitems_to_keep = var_map.loc[var_map['LINKSTO'] == 'labevents'].ITEMID
        labitems_to_keep = sorted(set([ str(i) for i in labitems_to_keep ]), key=int)


        print("  starting per-ICU db queries with %d subjects..." % (len(icuids_to_keep)))
        chartitems_sql = ','.join(chartitems_to_keep)
        labitems_sql = ','.join(labitems_to_keep)
        ...
        X_frames = []
        total_icus = len(icuids_to_keep)
        for idx, icu in enumerate(icuids_to_keep, 1):
            icu_query = query_template.format(icu=icu, chitem=chartitems_sql, lbitem=labitems_sql)
            df_chunk = querier.query(query_string=icu_query)
            if not df_chunk.empty:
                X_frames.append(df_chunk)
            if idx % 10 == 0 or idx == total_icus:
                print("    fetched %d/%d ICU stays" % (idx, total_icus))

        if X_frames:
            X = pd.concat(X_frames, ignore_index=True)
        else:
            X = pd.DataFrame(columns=['subject_id', 'hadm_id', 'icustay_id', 'charttime', 'itemid', 'value', 'valueuom'])
    ...
    data_for_hdf = data.copy()
    for col in data_for_hdf.columns:
        if pd.api.types.is_integer_dtype(data_for_hdf[col].dtype):
            data_for_hdf[col] = data_for_hdf[col].astype('int64')
    data_for_hdf.to_hdf(os.path.join(outPath, dynamic_hd5_filt_filename), 'patients', format='table')
